...
class WKANLinear(torch.nn.Module):
    def __init__(
        self,
        in_features,
        out_features,
        ...
        self.spline_weight = torch.nn.Parameter(
            torch.Tensor(1,in_features, grid_size + spline_order)            #这是最大的不同与KAN
        )
    ...
    def forward(self, x: torch.Tensor): # 将输入数据通过模型的各个层，经过线性变换和激活函数处理，最终得到模型的输出结果
        ...
        spline = self.b_splines(x)*self.spline_weight
        # import ipdb;ipdb.set_trace()
        spline = spline.sum(dim=-1)
        spline_output = F.linear(spline,self.spline_scaler)
...
class WKAN(torch.nn.Module): # 封装了一个WKAN神经网络模型，可以用于对数据进行拟合和预测。
    def __init__(
        self,
        layers_hidden,
        grid_size=5,
    ...
    def forward(self, x: torch.Tensor, update_grid=False): # 调用每个WKANLinear层的forward方法，对输入数据进行前向传播计算输出。
        """
        前向传播函数。
